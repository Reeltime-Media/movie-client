...
import { createSeriesUnlockBakongIntent, getPaymentIntent } from "@/lib/api/payments";
...
function nextPollDelayMs(elapsedMs: number): number {
  // Snappy unlock (~5–10s) while limiting NBC burn vs old 3s polls.
  if (elapsedMs < 180_000) return 5000;
  if (elapsedMs < 600_000) return 10000;
  return 20000;
}
...
export type SeriesUnlockBakongCheckoutModalProps = {
  seriesSlug: string;
  title: string;
  /** Where to send the viewer once the series is unlocked — defaults to the series' own watch page. */
  watchHref: string;
  onClose: () => void;
};

export function SeriesUnlockBakongCheckoutModal({
  seriesSlug,
  title,
  watchHref,
  onClose,
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const intent = await createSeriesUnlockBakongIntent(seriesSlug);
        if (closedRef.current || gen !== genRef.current) return;
      ...
      >
        ...
        {status === "succeeded" ? (
          <div className="flex w-[260px] flex-col items-center gap-3 rounded-xl border border-border bg-surface p-8 text-center">
            ...
            <p className="text-[12px] text-text-muted">Unlocking {title}…</p>
          </div>
          ...
          <>
            ...
            <p className="mt-1 max-w-[260px] text-center text-[12px] text-white/60">
              Scan with Bakong or any banking app to unlock {title}.
            </p>
          </>
        )}
      </div>
